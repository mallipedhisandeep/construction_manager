...
class _PrivateWorkListPageState extends State<PrivateWorkListPage> {
  ...
  List<PrivateWork> get _filtered {
    ...
    if (_search.trim().isNotEmpty) {
      ...
      list = list.where((w) => w.workerName.toLowerCase().contains(q) ||
        w.siteName.toLowerCase().contains(q) || w.workType.toLowerCase().contains(q)).toList();
    }
    return list;
  }

  Color _statusColor(String s) => switch (s.toLowerCase()) {
    'active' => Colors.green, 'completed' => Colors.blue, _ => Colors.grey,
  };
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Private Work', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: cs.primary, foregroundColor: Colors.white,
      ),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final ok = await Navigator.push(context, MaterialPageRoute(builder: (_) => const AddEditPrivateWorkPage()));
          if (ok == true) _load();
        },
              ...
              Container(color: Colors.white, padding: const EdgeInsets.all(12), child: Column(children: [
                ...
                if (_works.isNotEmpty) Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: totalBalance > 0 ? Colors.orange.shade200 : Colors.green.shade200)),
                  child: Row(children: [
                    ...
                    Text(
                      totalBalance > 0
                        ? 'Total Pending: ₹${totalBalance.toStringAsFixed(0)}'
                        : 'All Settled ✓',
                      style: TextStyle(
                ...
                TextField(
                  decoration: InputDecoration(
                    hintText: 'Search work entries...', prefixIcon: const Icon(Icons.search),
              ...
              Expanded(
                child: filtered.isEmpty
                  ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      Text('No work entries found', style: TextStyle(color: Colors.grey.shade500, fontSize: 16)),
                    ]))
                  : ListView.builder(
                          ...
                          child: InkWell(
                            ...
                            onTap: () async {
                              final changed = await Navigator.push(context, MaterialPageRoute(
                                builder: (_) => PrivateWorkDetailsPage(work: w)));
                              ...
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Row(children: [
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: sc.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                                    child: Text(w.status, style: TextStyle(fontSize: 12, color: sc, fontWeight: FontWeight.bold))),
                                ]),
                                ...
                                Row(children: [
                                  ...
                                  const SizedBox(width: 4),
                                  Text(w.siteName, style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                                ]),
                                const SizedBox(height: 8),
                                Row(children: [
                                  _moneyChip('Charged', w.priceCharged, Colors.blue),
                                  const SizedBox(width: 8),
                                  _moneyChip('Paid', w.amountPaid, Colors.green),
                                  const SizedBox(width: 8),
                                  if (balance > 0) _moneyChip('Due', balance, Colors.orange),
                                ]),
                              ]),
                            ),
                          ),
  ...
  Widget _moneyChip(String label, double amount, Color color) => Container(
